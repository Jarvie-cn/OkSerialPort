...
public class ResultDataParseUtils {
    ...
    public static boolean handleDataPack(ByteBuffer byteBuffer, byte[] received, int size, DataPackCallback dataPackCallback) {
        ...
        while ((readable = byteBuffer.remaining()) >= OkSerialPort_ProtocolManager.MINDALALEN) {
           ...
           a: for (int i = 0; i < OkSerialPort_ProtocolManager.FRAMEHEADERCOUNT; i++) {
...
//            LogPlus.e("原始字节数据：" +ByteUtil.bytes2HexStr(allPack) );
            ...
            if (OkSerialPort_ProtocolManager.CHECKCODERULE == 0) {
                ...
                if (dataToXOR.equalsIgnoreCase(currentXOR)) {
                    ...
                    byte[] data = ByteUtil.getBytes(allPack,OkSerialPort_ProtocolManager.DATASTARTINDEX, dataLen);
                    byte[] commands = ByteUtil.getBytes(allPack, OkSerialPort_ProtocolManager.COMMANDSTARTINDEX, mProtocolMap.get(OkSerialPort_ProtocolManager.COMMANDSTARTINDEX).length);
                    //最后清掉之前处理过的不合适的数据
                ...
                String recCheck = ByteUtil.bytes2HexStr(allPack, total - oXRLen, mProtocolMap.get(mProtocolMap.size()-1).length).toUpperCase();
